...
int sep = 0;// Separate(A, A_size, B, B_size, C);
	...
	if (sep == 0) {
		...
		while (i < size_C)
		{
			...
			else if (A[0]>B[0]) {AddAndPop(C,&C_size,&B,&size_B); printf("%s\n", "Case1");}

			else if (B[0]>A[0]) {AddAndPop(C,&C_size,&A,&size_A); printf("%s\n", "Case2");}
...
void main() {
	int A[2]={1,5}; const int A_size=2; int* ptrA=&A[0];
